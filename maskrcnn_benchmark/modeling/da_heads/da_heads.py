...
class DAInsHead(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        # Modify it to have an output layer with (number of classes + 1) nodes, 
        # where each node represents a binary classification (source/target) for the corresponding class instance domain
        # self.fc3_da = nn.Linear(1024, 17)
        # Number of classes + 1, where the 0-th node represents the background class, the i-th node represents the i-th class, a value of 0 indicates it belongs to the non-source domain, and a value of 1 indicates the source domain
        self.fc3_da = nn.Linear(1024, 17)
...
class DomainAdaptationModule(torch.nn.Module):
    ...
    def forward(self, img_features, da_ins_feature, da_ins_labels, targets=None, ins_cls_labels=None):
        """
        Arguments:
        ...
        if self.training:
            da_img_loss, da_ins_loss, da_consistency_loss = self.loss_evaluator(
                da_img_features, da_ins_features, da_img_consist_features, da_ins_consist_features, da_ins_labels, targets, ins_cls_labels
            )
            losses = {}
